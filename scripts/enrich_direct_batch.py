#!/usr/bin/env python3
"""
Direct property site enrichment with batched processing.
Uses Daft.ie → MyHome.ie cascade with batch delays to avoid blocking.

Usage:
    python3 scripts/enrich_direct_batch.py --months 3 --batch-size 100 --batch-delay 180
"""
...
def cascade_search(address, county):
    """
    Try Daft.ie then MyHome.ie until success.
    """
    sources = [
    ...
    for i, (source_name, search_func, delay) in enumerate(sources):
        ...
        if result.get('skip_source'):
            print(f"   ⚠️  {source_name} blocked/rate-limited, skipping remaining sources", flush=True)
            break
...
def enrich_batch(properties, batch_num, total_batches, dry_run=False):
    ...
    print(f"\n{'='*80}", flush=True)
    print(f"BATCH {batch_num}/{total_batches} - Processing {len(properties)} properties", flush=True)
...
def enrich_properties(months=3, batch_size=100, batch_delay=180, dry_run=False):
    """
    Enrich properties in batches with delays.

    Args:
        months: How many months back to search
        batch_size: Properties per batch (default: 100)
        batch_delay: Seconds between batches (default: 180 = 3 minutes)
        dry_run: If True, don't update database
    """
    print("="*80, flush=True)
    print("BATCHED DIRECT-SITE ENRICHMENT (Daft.ie → MyHome.ie)", flush=True)
    print("="*80, flush=True)
    ...
    try:
        ...
        for batch_num, batch in enumerate(batches, 1):
            ...
            if batch_num < total_batches:
                print(f"\n   ⏸️  Pausing {batch_delay} seconds before next batch...", flush=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch-size', type=int, default=100, help='Properties per batch')
