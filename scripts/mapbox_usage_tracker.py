...
class MapboxUsageTracker:
    ...
    async def save(self):
        """Persist accumulated usage via a fresh, short-lived connection.

        A dedicated connection is used (NOT self._conn) because self._conn is opened
        at start() and may sit idle for the entire geocoding phase — on a long batch
        run Supabase closes it server-side, and the exit-time save then failed with
    ...
    async def close(self):
        ...
        Usage tracking must NEVER abort the caller: a failed save here previously
        propagated out of the batch's `async with`, discarding a whole sub-batch's
        already-paid-for geocoding results. Swallow save errors (they are logged) so
        the caller can still write its results.
        """
        try:
            await self.save()
